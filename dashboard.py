...
st.set_page_config(page_title="Flight Monitor", page_icon="✈️", layout="wide", initial_sidebar_state="expanded")
...
.tag-alert { display:inline-block; background:#1a0a0a; border:1px solid #4a1a1a; border-radius:4px; padding:2px 8px; font-size:0.7rem; color:#ff4757; letter-spacing:0.08em; text-transform:uppercase; }
.tag-ok { display:inline-block; background:#0a1a0a; border:1px solid #1a4a1a; border-radius:4px; padding:2px 8px; font-size:0.7rem; color:#2ed573; letter-spacing:0.08em; text-transform:uppercase; }
...
AIRPORTS = [
    ...
    ("RHO","Rodi","Grecia"), ("CFU","Corfu","Grecia"),
    ("ZTH","Zante","Grecia"), ("JMK","Mykonos","Grecia"),
    ("DUB","Dublino","Irlanda"), ("CPH","Copenaghen","Danimarca"),
    ("ARN","Stoccolma Arlanda","Svezia"), ("OSL","Oslo","Norvegia"),
    ("HEL","Helsinki","Finlandia"), ("WAW","Varsavia","Polonia"),
    ("PRG","Praga","Rep. Ceca"), ("BUD","Budapest","Ungheria"),
    ("IST","Istanbul","Turchia"), ("SAW","Istanbul Sabiha","Turchia"),
    ("AYT","Antalya","Turchia"), ("DXB","Dubai","UAE"),
    ("AUH","Abu Dhabi","UAE"), ("DOH","Doha","Qatar"),
    ...
    ("JNB","Johannesburg","Sud Africa"), ("CPT","Citta del Capo","Sud Africa"),
    ("NBO","Nairobi","Kenya"), ("ZNZ","Zanzibar","Tanzania"),
    ("JFK","New York JFK","USA"), ("EWR","New York Newark","USA"),
    ("LAX","Los Angeles","USA"), ("ORD","Chicago","USA"),
    ("MIA","Miami","USA"), ("SFO","San Francisco","USA"),
    ("BOS","Boston","USA"), ("LAS","Las Vegas","USA"),
    ("MCO","Orlando","USA"), ("IAD","Washington","USA"),
    ("YYZ","Toronto","Canada"), ("YVR","Vancouver","Canada"),
    ("CUN","Cancun","Messico"), ("HAV","L Avana","Cuba"),
    ("PUJ","Punta Cana","Rep. Dominicana"),
    ("NRT","Tokyo Narita","Giappone"), ("HND","Tokyo Haneda","Giappone"),
    ("KIX","Osaka Kansai","Giappone"), ("ICN","Seoul Incheon","Corea del Sud"),
    ("HKG","Hong Kong","Hong Kong"), ("SIN","Singapore Changi","Singapore"),
    ("BKK","Bangkok","Tailandia"), ("HKT","Phuket","Tailandia"),
    ...
    ("GRU","Sao Paulo","Brasile"), ("EZE","Buenos Aires","Argentina"),
]
...
def generate_dates(start, end, max_n=5):
    delta = (end - start).days
    if delta <= 0:
        return [str(start)]
    if delta < max_n:
        return [str(start + timedelta(days=i)) for i in range(delta + 1)]
    step = delta // (max_n - 1)
    dates = [str(start + timedelta(days=i * step)) for i in range(max_n)]
    if str(end) not in dates:
        dates[-1] = str(end)
    return dates
...
def flight_key(f):
    return f["origin"] + "-" + f["destination"] + "-" + f["date"]
...
def check_env():
    missing = []
    for v in ["SERPAPI_KEY", "SMTP_USER", "SMTP_PASSWORD"]:
...
with st.sidebar:
    ...
    if orig_q:
        ...
        if res:
            opts = ["— seleziona —"] + [fmt_airport(c, n, co) for c, n, co in res]
            ch = st.selectbox("Partenza risultati", opts, key="orig_box", label_visibility="collapsed")
            if ch != "— seleziona —":
                code = ch.split(" — ")[0]
                for r in res:
                    if r[0] == code:
                        st.session_state.orig_sel = r
                        break
        else:
    ...
    if dest_q:
        ...
        if res2:
            opts2 = ["— seleziona —"] + [fmt_airport(c, n, co) for c, n, co in res2]
            ch2 = st.selectbox("Destinazione risultati", opts2, key="dest_box", label_visibility="collapsed")
            if ch2 != "— seleziona —":
                code2 = ch2.split(" — ")[0]
                for r2 in res2:
                    if r2[0] == code2:
                        st.session_state.dest_sel = r2
                        break
        else:
            st.caption("Nessun risultato")
    ...
    with col2:
        d_to = st.date_input("Al", value=date.today() + timedelta(37), min_value=date.today(), key="d_to", label_visibility="collapsed")
    ...
    if use_ret:
        ...
        with col4:
            r_to = st.date_input("Ritorno al", value=d_to + timedelta(14), min_value=d_from, key="r_to", label_visibility="collapsed")
    else:
        r_from = None
        r_to = None
    ...
    if st.button("Aggiungi volo", use_container_width=True, type="primary"):
        ...
        if not orig:
            st.error("Seleziona aeroporto di partenza")
        elif not dest:
            st.error("Seleziona aeroporto di destinazione")
        elif orig[0] == dest[0]:
            st.error("Partenza e destinazione uguali")
        elif d_to < d_from:
            ...
            st.session_state.orig_sel = None
            st.session_state.dest_sel = None
            st.success("Aggiunto! " + str(added) + " combinazione/i di date.")
            st.rerun()
...
""", unsafe_allow_html=True)
st.markdown('<div class="meta-label">' + str(len(flights)) + ' tratte monitorate · ' + datetime.now().strftime("%d/%m/%Y %H:%M") + '</div>', unsafe_allow_html=True)
st.markdown("<br>", unsafe_allow_html=True)

tab_flights, tab_notifications, tab_log = st.tabs(["Voli", "Notifiche", "Log"])


# ── TAB VOLI ─────────────────────────────────────────────────────────────────
with tab_flights:
    if not flights:
        st.markdown('<div style="background:#0f0f1a;border:1px dashed #2a2a3a;border-radius:12px;padding:40px;text-align:center;color:#333;margin-top:1rem;"><div style="font-size:1.5rem;margin-bottom:8px;">✈️</div><div>Nessun volo monitorato. Cerca un aeroporto dal pannello laterale.</div></div>', unsafe_allow_html=True)
    else:
        ...
        for i, f in enumerate(flights):
            ...
            if lbl2 not in groups:
                groups[lbl2] = []
            groups[lbl2].append((i, f))
        ...
        for label, group in groups.items():
            ...
            for i, f in group:
                ...
                is_alert = bool(s["current"] and f.get("max_price") and s["current"] <= f["max_price"])
                ...
                with cp:
                    if s["current"]:
                        if is_alert:
                            ...
                            pc = "#e8e8f0"
                        st.markdown('<div class="flight-card" style="text-align:center;"><div class="meta-label">prezzo attuale</div><div style="font-family:Syne,sans-serif;font-size:2.2rem;font-weight:800;color:' + pc + ';line-height:1.1;">EUR' + str(s["current"]) + '</div><div style="color:' + t_color + ';font-size:1rem;margin-top:4px;">' + t_icon + '</div></div>', unsafe_allow_html=True)
                    else:
                ...
                with cs:
                    if s["current"]:
                        ...
                        if s["last_check"]:
                            try:
                                lc = datetime.fromisoformat(s["last_check"]).strftime("%d/%m %H:%M")
                            except Exception:
                                lc = s["last_check"][:16]
                        st.markdown('<div class="flight-card"><div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;"><div><div class="meta-label">minimo</div><div style="font-family:Syne,sans-serif;font-weight:700;color:#2ed573;">EUR' + str(s["min"]) + '</div></div><div><div class="meta-label">massimo</div><div style="font-family:Syne,sans-serif;font-weight:700;color:#ff4757;">EUR' + str(s["max"]) + '</div></div><div><div class="meta-label">media</div><div style="font-family:Syne,sans-serif;font-weight:700;">EUR' + str(s["avg"]) + '</div></div><div><div class="meta-label">ultimo check</div><div style="font-size:0.75rem;color:#666;">' + lc + '</div></div></div></div>', unsafe_allow_html=True)
                    else:
                ...
                with cd:
                    ...
                    if st.button("X", key="del_" + str(i), help="Rimuovi"):
                ...
                if s["history"] and len(s["history"]) >= 2 and has_plotly:
                    ...
                    fig.add_trace(go.Scatter(x=xs, y=ys, mode='lines+markers', line=dict(color='#2a52be', width=2), marker=dict(size=5, color='#4a72de', line=dict(color='#0a0a0f', width=1)), hovertemplate='EUR%{y}<br>%{x}<extra></extra>', showlegend=False))
                    if f.get("max_price"):
                        fig.add_hline(y=f["max_price"], line_dash="dash", line_color="#ff4757", line_width=1, annotation_text="soglia EUR" + str(f["max_price"]), annotation_font_color="#ff4757", annotation_font_size=10)
                    fig.update_layout(height=100, margin=dict(l=0, r=0, t=0, b=0), paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', xaxis=dict(showgrid=False, zeroline=False, tickfont=dict(size=9, color='#444')), yaxis=dict(showgrid=True, gridcolor='#1a1a2a', zeroline=False, tickfont=dict(size=9, color='#444'), tickprefix='EUR'))
...
with tab_notifications:
    ...
    if recipients:
        for idx, email in enumerate(recipients):
            ...
            with ce:
                st.markdown('<div style="background:#0f0f1a;border:1px solid #1e1e30;border-radius:8px;padding:10px 14px;font-size:0.82rem;color:#aab;margin-bottom:6px;">📧 ' + email + '</div>', unsafe_allow_html=True)
            with cr:
                if st.button("X", key="rm_" + str(idx)):
    ...
    with st.form("add_r", clear_on_submit=True):
        ...
        with ci2:
            new_em = st.text_input("Email", placeholder="mario@example.com", label_visibility="collapsed")
        with cb2:
            add_r = st.form_submit_button("Aggiungi", use_container_width=True)
        if add_r:
            e = new_em.strip().lower()
            if "@" not in e or "." not in e:
                st.error("Email non valida")
            elif e in config.get("notification_recipients", []):
                st.warning("Gia presente")
            else:
                if "notification_recipients" not in config:
                    config["notification_recipients"] = []
                config["notification_recipients"].append(e)
                save_config(config)
                st.success("Aggiunto: " + e)
                st.rerun()
    ...
    with ct1:
        if recipients:
            test_r = st.selectbox("Invia a", recipients)
        else:
            st.selectbox("Invia a", ["— nessun destinatario —"], disabled=True)
            test_r = None
    with ct2:
        st.markdown("<div style='height:28px'></div>", unsafe_allow_html=True)
        send_t = st.button("Invia test", disabled=not recipients, use_container_width=True)

    if send_t and recipients and test_r:
        ev_ok, ev_miss = check_env()
        if not ev_ok:
            st.error("Variabili mancanti: " + ", ".join(ev_miss))
        else:
            su = os.environ.get("SMTP_USER")
            sp = os.environ.get("SMTP_PASSWORD")
            html = '<div style="font-family:Arial,sans-serif;max-width:480px;margin:20px auto;background:#1a1a2e;border-radius:12px;padding:30px;color:white;"><div style="font-size:22px;font-weight:800;margin-bottom:10px;">Flight Monitor</div><div style="background:#0a0a1a;border-radius:8px;padding:16px;color:#2ed573;">Configurazione email funzionante.</div><div style="color:#555;font-size:12px;margin-top:16px;">' + datetime.now().strftime("%d/%m/%Y %H:%M") + '</div></div>'
            try:
                msg = MIMEMultipart("alternative")
                msg["Subject"] = "Flight Monitor — Test"
                msg["From"] = "Flight Monitor <" + su + ">"
                msg["To"] = test_r
                ...
                with smtplib.SMTP("smtp.gmail.com", 587) as srv:
                    ...
                    srv.starttls()
                    srv.login(su, sp)
                    srv.sendmail(su, [test_r], msg.as_string())
                st.success("Email inviata a " + test_r)
            except Exception as ex:
                st.error("Errore: " + str(ex))
    ...
    with cl2:
        ...
        if st.button("Pulisci log", use_container_width=True):
            if LOG_FILE.exists():
                LOG_FILE.write_text("")
            st.rerun()
    st.markdown('<div class="log-box">' + load_log(100) + '</div>', unsafe_allow_html=True)
